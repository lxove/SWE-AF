...
# ---------------------------------------------------------------------------
# Push + PR creation: push branch after improve loop, optionally create PR
# ---------------------------------------------------------------------------


def _make_improve_call_mock(pr_response=None, pr_raises=None):
    """Helper: create an AsyncMock for app.call that handles validate/execute/pr calls."""
    async def mock_call(*args, **kwargs):
        ...
        if "validate" in call_name:
            ...
            if pr_raises:
                raise pr_raises
            return pr_response or {}
        return {}
    return AsyncMock(side_effect=mock_call)


@pytest.mark.asyncio
async def test_branch_pushed_after_successful_improvements(temp_repo, sample_improvement):
    """After completing improvements, improve() should push the branch to remote."""
    state = ImprovementState(
        repo_path=temp_repo,
        improvements=[sample_improvement],
        last_scan_at=datetime.now(timezone.utc).isoformat(),
...
@pytest.mark.asyncio
async def test_pr_failure_is_non_fatal_branch_still_pushed(temp_repo, sample_improvement):
    """If PR creation fails, branch is still pushed and result succeeds."""
    state = ImprovementState(
        repo_path=temp_repo,
        improvements=[sample_improvement],
        last_scan_at=datetime.now(timezone.utc).isoformat(),
